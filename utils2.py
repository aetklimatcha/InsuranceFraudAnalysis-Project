...
def plot_two_categorical(df, column1, column2, plot_type="barplot"):
    ...
    if plot_type == "barplot":
        ...
        crosstab_melted = crosstab.melt(
            id_vars=column1, value_vars=crosstab.columns[1:]
        )
        ...
        fig = px.bar(
            ...
            barmode="group",
            labels={"value": "Count"},
            title=f"Counts of {column1} and {column2} Combinations",
        )

    elif plot_type == "stacked_barplot":
        crosstab_percentage = pd.crosstab(
            df[column1], df[column2], normalize="index"
        ).reset_index()
        crosstab_melted = crosstab_percentage.melt(
            id_vars=column1, value_vars=crosstab_percentage.columns[1:]
        )
        ...
        fig = px.bar(
            ...
            title=f"100% Stacked Barplot of {column1} and {column2} Combinations",
        )
        ...
        fig = px.imshow(
            crosstab,
            text_auto=True,
            color_continuous_scale=["#CBE2B5", "#86AB89"],
            labels={"color": "Count"},
            title=f"Heatmap of {column1} and {column2} Combinations",
        )
    else:
...
def plot_count_plot(
    df: pd.DataFrame, column_name: str, color_column: str = None
) -> go.Figure:
    """
    ...
    if color_column is None:
        ...
        for i in range(num_plots):
            fig.update_xaxes(
                categoryorder="array", categoryarray=category_order, row=1, col=i + 1
            )
...
def plot_observed_vs_expected(array1, array2):
    ...
    observed_melted = contingency_table.reset_index().melt(
        id_vars=contingency_table.index.name
    )
